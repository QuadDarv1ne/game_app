...
<head>
    ...
    <title>Редактирование: {{ $post->title }} - {{ config('app.name', 'GameApp') }}</title>
...
<main class="flex-grow max-w-4xl mx-auto px-6 py-12 w-full space-y-10 relative overflow-hidden">
    ...
    <div class="relative z-10 font-mono text-xs flex items-center justify-between">
        <a href="{{ route('posts.show', $post) }}" class="text-zinc-500 hover:text-red-400 transition-colors uppercase tracking-wider">
            [ ← Вернуться к статье ]
        </a>
        <span class="text-zinc-600 text-[10px] uppercase tracking-widest">// Режим редактирования</span>
    </div>
    ...
    <form action="{{ route('posts.update', $post) }}" method="POST" class="relative z-10 space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-2">
            ...
            <select name="category_id"
                    ...
                    required>
                <option value="" class="bg-zinc-950">Выберите категорию</option>
        ...
        <div class="space-y-2">
            ...
            <textarea name="description"
                      ...
                      placeholder="Краткое описание поста (до 255 символов)...">{{ old('description', $post->description) }}</textarea>
            <div class="flex justify-between">
                <span class="text-[10px] font-mono text-zinc-600">// Макс. 255 символов</span>
                <span class="text-[10px] font-mono text-zinc-600" id="description-count">{{ strlen(old('description', $post->description ?? '')) }}/255</span>
            </div>
            @error('description')
            <span class="text-[10px] font-mono text-red-500">// Ошибка: {{ $message }}</span>
            @enderror
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (excerptInput && excerptCount) {
            excerptInput.addEventListener('input', function() {
                ...
                excerptCount.textContent = length + '/255';
                excerptCount.style.color = length > 255 ? '#ef4444' : '#52525b';
            });
        }
    });
</script>
